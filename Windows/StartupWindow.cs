...
namespace TEKLauncher.Windows
{
    public partial class StartupWindow : Window
    {
        ...
        {
            ContinueButton.Content = (InstallMode = !FileExists($@"{Path = Selector.Text}\ShooterGame\Binaries\Win64\ShooterGame.exe")) ? "Install" : "Continue";
            ...
            if (InstallMode)
            {
                ...
                Path += @"\ArkGameData";
                FreeDiskSpace.Foreground = (ContinueButton.IsEnabled = FreeSpace > 118111600639L) ? DarkGreen : DarkRed;
                FreeDiskSpace.Text = ConvertBytes(FreeSpace);
            }
            else
                ContinueButton.IsEnabled = true;
        }
